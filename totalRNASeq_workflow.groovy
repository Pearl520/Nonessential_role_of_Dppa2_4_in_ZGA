title: "total RNA-Seq analysis pipeline "

DataDir =  "/n/scratch3/users/z/zc79/Dppa2_4_RNAseq/raw_data"

def branches = [

     sample1 : ["${DataDir}/sample1_R1.fastq.gz",
                "${DataDir}/sample1_R2.fastq.gz"],

     sample2 : ["${DataDir}/sample2_R1.fastq.gz",
                "${DataDir}/sample2_R2.fastq.gz"]
...
Trimgalor = {
    ...
    produce("./Trimgalor/${branch.name}_trimgalor/${basename1}_val_1.fq.gz","./Trimgalor/${branch.name}_trimgalor/${basename2}_val_2.fq.gz"){
     exec """
       ...
       trim_galore --clip_R2 5  --paired  --fastqc --output_dir "Trimgalor/${branch.name}_trimgalor" $inputs
     ""","Trimgalor"
    }
}

bowtie2_rRNA_map = {
        def bowtie2_index = "/n/groups/zhanglab/zychen/genomes_annotations/rRNA_tRNA_db/Mus_rRNA_tRNA"
        def nbcpu = 4
        def outdir = "${branch.name}_bowtie2_rRNA"

        produce("${outdir}/${branch.name}.unmapped.bowtie2.1.gz","${outdir}/${branch.name}.unmapped.bowtie2.2.gz"){
               exec """
               module load gcc boost bowtie2/2.2.9 samtools/1.9 &&
               mkdir -p ${outdir} &&
               bowtie2	--no-unal 
			-I 0 
			...
			-2 ${input2}
			-p ${nbcpu}
			 --un-conc-gz ${outdir}/${branch.name}.unmapped.bowtie2.gz
			 | samtools view -bS - > ${outdir}/${branch.name}.bam 
               ""","bowtie2_rRNA_map"
        }
 }
